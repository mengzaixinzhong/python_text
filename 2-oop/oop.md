...
- 你如何声明一个类
 ...
 - MIxin设计模式
   - 主要采用多继承方式对类的功能进行扩展
   -【mixin概念/MRO and Mixin/MIxin模式/Mixin MRO/MRo】详见网页收藏
 - 我们使用多继承语法来实现Mixin
 - 使用Mixin实现多继承的时候非常小心
 ...
 # 4 类相关函数
 - issubclass(B,A)查看b是不是a的子类
 - isinstance(a,A):检测一个对象是否是这个类的
 - hasattr(a，属性名):检测一个对象是否含有某个属性
 - getattr: get attribute
 - setattr:set attribute
 - delattr;delete attribute
 - dir:获取对象的成员列表
 # 5. 类的成员描述符（属性）
 - 类的成员描述福是为了在类中对累的成员属性进行相关操作二创建的一种方式
   - get 获取属性的操作
   - set  修改或者添加属性操作
   - dekete 删除属性的操作
 - 如果想使用类的成员描述符，大概有三种办法在：
   - 使用类实现描述器
   - 使用属性修饰符
   - 使用property函数
     - property函数很简单
